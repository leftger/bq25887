...
#[command(
    name = "generate-api",
    about = "Generate the BQ25887 Rust API from the YAML device manifest.",
    long_about = "Reads the BQ25887 device manifest and produces the Rust driver scaffolding by leveraging the device-driver-generation crate."
)]
struct Cli {
    #[arg(
        short,
        long,
        value_name = "PATH",
        default_value = "src/bq25887.yaml",
...
fn prettify(source: &str) -> Result<String> {
    ...
    let syntax_tree: File = syn::parse2(tokens).context("Unable to convert tokens into a syntax tree")?;
...
fn add_header(formatted: &str, manifest_path: &Path) -> String {
    ...
    output.push_str(&format!("// Source manifest: {}\n\n", manifest_path.display()));
